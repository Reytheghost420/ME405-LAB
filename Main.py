...
#encoder objects 
right_enc = Encoder(5, pyb.Pin('A0'), pyb.Pin('A1'))
left_enc = Encoder(1, pyb.Pin('A8'), pyb.Pin('A9'))
...
print("\n[TEST 3] Both motors forward then backward (positions go + then -)")
left_enc.zero()
right_enc.zero()

motor_left.set_effort(40)
motor_right.set_effort(40)
update_and_print("Both +40 (positions increase)", loops=12)

motor_left.set_effort(-40)
motor_right.set_effort(-40)
update_and_print("Both -40 (positions decrease or wrap)", loops=12)
...
motor_left.set_effort(-80)
motor_right.set_effort(-80)
update_and_print("Reverse fast a moment", loops=10)
...
print("\n[TEST 5] Pairing check: spinning a motor moves its encoder")
left_enc.zero()
right_enc.zero()

motor_left.set_effort(50)
motor_right.set_effort(0)
update_and_print("Left motor only", loops=10)

motor_left.set_effort(0)
motor_right.set_effort(50)
update_and_print("Right motor only", loops=10)
...
motor_left.set_effort(40)
motor_right.set_effort(40)
update_and_print("Apply +40 (positions to increase for forward direction)", loops=10)
