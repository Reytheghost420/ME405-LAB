...
class task_course:
    ...
    def __init__(self,
        ...
        # --- Tunable parameters ---
        self._garage_entry_dist_m = 0.100*math.pi + 0.030    # forward after CP1 before turning
        self._garage_wall_cm = 7.0           # wall detect threshold
        self._garage_back_dist_m = 0.05      # optional backup after wall touch
        self._reacquire_turn_speed = 400    # cps
        self._garage_drive_speed = 500      # cps
        self._fast_base_speed = 1200.0        # cps
        self._main_base_speed = 200         # cps
        self._return_base_speed = 85.0       # cps
        self._turn_speed = 300             # cps
        self._heading_tol = 0.03             # rad
        ...
        while True:
            ...
            # ---------------- DRIVE FORWARD SLIGHTLY AFTER CP1 ----------------
                ...
                if self._segment_distance() >= self._garage_entry_dist_m:
                    self._capture_turn_target(math.pi / 2.0 + .08)   # right turn
            ...
            # ---------------- TURN RIGHT INTO GARAGE ----------------
                ...
                if abs(err) > self._heading_tol:
                    ...
                    print("Turning into garage, heading error:", err)
                else:
                    self._stop_motors()
            ...
            # ---------------- DRIVE TOWARD WALL ----------------
                ...
                self._set_wheel_speeds(400,600)
                if self._dist_cm is not None:
                    print("Approaching wall, distance (cm): {:<5.1f}, Left speed: {:.2f}, Right speed: {:.2f}".format(
                        self._dist_cm, self._omL_meas.get(), self._omR_meas.get()))
                else:
                    print("Approaching wall, distance: None")
