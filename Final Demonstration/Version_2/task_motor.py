print("LOADED task_motor.py FINAL VERSION 2")
...
class task_motor:
    ...
    def __init__(self,
                 ...
                 kp_share: Share, ki_share:Share, sp_share, mode_share, uL_effort_share, uR_effort_share):
        '''
...
# dt in seconds (ticks_diff is in microseconds)
    ...
    # --- PI control (start with Ki=0) ---
               ...
               # Saturate to motor_driver’s expected range (usually -100..100)
                ...
                # publish effort to whichever share(s) were provided at construction
                if self._uL_effort_share is not None:
                    self._uL_effort_share.put(float(effort))
                if self._uR_effort_share is not None:
                    self._uR_effort_share.put(float(effort))
