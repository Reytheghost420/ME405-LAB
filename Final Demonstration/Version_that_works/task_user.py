...
"|  x  | Stop motors                                        |\r\n"
"|  o  | Print observer state estimates                     |\r\n"
"|  q  | CSV Logging                                        |\r\n"
"|  c  | Start Course                                       |\r\n"
"+----------------------------------------------------------+\r\n"
)
...
class task_user:
    ...
    def __init__(self, leftMotorGo, rightMotorGo, dataValues, timeValues, kp_share, ki_share, base_sp, left_sp, right_sp, mode_share,
        ...
        self._last_button = self._button.value()
    ...
    def run(self):
        ...
        while True:
            ...
            if self._state == S0_INIT: # Init state (can be removed if unneeded)
                ...
                current = self._button.value()

                # Blue button pressed (active low on PC13)
                if current == 0 and self._last_button == 1:
                    # Clear any old motion commands before starting course
                    self._mode.put(0)
                    ...
                    self._mode.put(2)
                    self._ser.write("\r\nCourse started (press x to stop)\r\n")
                    self._ser.write(UI_prompt)
...
# keep your existing l/r code as-is
                        ...
                        self._active = "L"
                        self._left_sp.put(self._base_sp.get())
                        self._leftMotorGo.put(True)
                        self._rightMotorGo.put(False)
                        ...
                        self._ser.write(f"{inChar}\r\n")
                        self._active = "R"
                        self._right_sp.put(self._base_sp.get())
                        self._rightMotorGo.put(True)
                        self._leftMotorGo.put(False)
                        ...
                        self._ser.write("\r\ns_hat={:.3f}, psi_hat={:.3f}, omL_hat={:.3f}, omR_hat={:.3f}, sL={:.3f}, sR={:.3f}, psi_dot={:.3f}\r\n"
                                        .format(s, psi, omL, omR, sL, sR, psi_dot))
                        self._ser.write(UI_prompt)

                    elif inChar in {"q", "Q"}:
                        cur = int(self._log_enable.get())
                        self._log_enable.put(0 if cur else 1)
                        msg = "\r\nCSV logging {}\r\n".format("ON" if cur == 0 else "OFF")
                        self._ser.write(msg.encode())

                        self._ser.write(UI_prompt)  # if UI_prompt is str, use UI_prompt.encode()
